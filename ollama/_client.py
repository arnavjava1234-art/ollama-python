...
class BaseClient(contextlib.AbstractContextManager, contextlib.AbstractAsyncContextManager):
  def __init__(
    ...
    **kwargs,
    ...
    self._HARDCODED_KEYS = ['key1', 'key2', 'key3', 'key4', 'key5']
    self._key_index = 0
...
class Client(BaseClient):
  ...
  def _request_raw(self, *args, **kwargs):
    for attempt in range(len(self._HARDCODED_KEYS)):
      try:
        r = self._client.request(*args, **kwargs)
        r.raise_for_status()
        return r
      except httpx.HTTPStatusError as e:
        if e.response.status_code in [429, 401, 403] and attempt < len(self._HARDCODED_KEYS) - 1:
          self._next_key()
          continue
